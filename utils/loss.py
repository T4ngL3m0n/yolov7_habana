...
def LogFileSetting(LogPath):
    if not os.path.exists(LogPath):
        os.makedirs(LogPath)
    LogTime = datetime.now().strftime("%Y-%m-%d")
    # logzero.logfile("/dev/stdout")
    logzero.logfile(f"{LogPath}/yolov9_{LogTime}.log")
LogFileSetting('Log')
...
class SigmoidBin(nn.Module):
    ...
    def training_loss(self, pred, target):
        ...
        target_bins = torch.full_like(pred_bin, self.cn, device=torch.device("hpu"))  # targets
        n = pred.shape[0] 
...
class RankSort(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta_RS=0.50, eps=1e-10): 
        ...
        classification_grads=torch.zeros(logits.shape).to(torch.device('hpu'))
        ...
        relevant_bg_grad=torch.zeros(len(relevant_bg_logits)).to(torch.device('hpu'))
        sorting_error=torch.zeros(fg_num).to(torch.device('hpu'))
        ranking_error=torch.zeros(fg_num).to(torch.device('hpu'))
        fg_grad=torch.zeros(fg_num).to(torch.device('hpu'))
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5): 
        ...
        relevant_bg_logits=logits[relevant_bg_labels] 
        relevant_bg_grad=torch.zeros(len(relevant_bg_logits)).to(torch.device('hpu'))
        rank=torch.zeros(fg_num).to(torch.device('hpu'))
        prec=torch.zeros(fg_num).to(torch.device('hpu'))
        fg_grad=torch.zeros(fg_num).to(torch.device('hpu'))
...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        ai = torch.arange(na, device=targets.device, dtype=torch.float32).view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        g = 0.5  # bias
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
                            # [1, 1], [1, -1], [-1, 1], [-1, -1],  # jk,jm,lk,lm
                            ], device=targets.device, dtype=torch.float32) * g  # offsets
        for i in range(self.nl):
            ...
            b, c = torch.tensor(t[:, :2], dtype=torch.int64, device=torch.device("hpu")).T  # image, class
            
            gxy = t[:, 2:4]  # grid xy
            gwh = t[:, 4:6]  # grid wh
            # gij = (gxy - offsets).long()
            gij = torch.tensor((gxy - offsets), dtype=torch.int64, device=torch.device("hpu"))
            
            gi, gj = gij.T  # grid xy indices

            # Append
            # a = t[:, 6].long()  # anchor indices
            a = torch.tensor(t[:, 6], dtype=torch.int64, device=torch.device("hpu"))
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for i in range(nl):
            if matching_targets[i] != []:
                ...
                matching_bs[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
                matching_as[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
                matching_gjs[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
                matching_gis[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
                matching_targets[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
                matching_anchs[i] = torch.tensor([], device=torch.device('hpu'), dtype=torch.int64)
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=torch.device("hpu")))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=torch.device("hpu")))
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        lcls, lbox, lobj = torch.zeros(1, device=torch.device("hpu")), torch.zeros(1, device=torch.device("hpu")), torch.zeros(1, device=torch.device("hpu"))
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, imgs)
        pre_gen_gains = [torch.tensor(pp.shape, device=torch.device("hpu"))[[3, 2, 3, 2]] for pp in p] 
    

        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = bs[i], as_[i], gjs[i], gis[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=torch.device("hpu"))  # target obj
            ...
            if n:
                ...
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, (1+obj_idx):], self.cn, device=torch.device("hpu"))  # targets
                    t[range(n), selected_tcls] = self.cp
    ...
    def find_3_positive(self, p, targets):
        ...
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        indices, anch = [], []
        gain = torch.ones(7, device=torch.device("hpu")).long()  # normalized to gridspace gain
        ai = torch.arange(na, device=torch.device("hpu")).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        ...
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
                            # [1, 1], [1, -1], [-1, 1], [-1, -1],  # jk,jm,lk,lm
                            ], device=torch.device("hpu")).float() * g  # offsets
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        pre_gen_gains_aux = [torch.tensor(pp.shape, device=torch.device("hpu"))[[3, 2, 3, 2]] for pp in p[:self.nl]] 
        pre_gen_gains = [torch.tensor(pp.shape, device=torch.device("hpu"))[[3, 2, 3, 2]] for pp in p[:self.nl]] 
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            tobj = torch.zeros_like(pi[..., 0], device=torch.device("hpu"))  # target obj
            tobj_aux = torch.zeros_like(pi_aux[..., 0], device=torch.device("hpu"))  # target obj
            ...
            if n:
                ...
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 5:], self.cn, device=torch.device("hpu"))  # targets
                    t[range(n), selected_tcls] = self.cp
                    lcls += self.BCEcls(ps[:, 5:], t)  # BCE
            ...
            if n_aux:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t_aux = torch.full_like(ps_aux[:, 5:], self.cn, device=torch.device("hpu"))  # targets
...
#         self.no = m.no
#         # logager.info(f"m.nc | {m.nc}")
#         # loagger.info(f"m.nl | {m.nl}")
#         # logager.info(f"m.no | {m.no}")
...
#         iou = bbox_iou_yolov9(pred_bboxes_pos, target_bboxes_pos, xywh=False, CIoU=True)
#         loss_iou = 1.0 - iou
...
# def bbox_iou_yolov9(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
